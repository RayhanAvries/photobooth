...
app.post('/api/print-temp', express.json({ limit: '20mb' }), async (req, res) => {
  ...
  try {
    ...
    const info = db.prepare('INSERT INTO print_jobs (image_path, printer_name, status) VALUES (?, ?, ?)')
      .run(`/print/${filename}`, printName, 'pending');
...
app.get('/api/print-jobs', (req, res) => {
  ...
  let jobs;
  if (status) {
    jobs = db.prepare('SELECT * FROM print_jobs WHERE status = ? ORDER BY created_at ASC LIMIT 10').all(status);
  } else {
    jobs = db.prepare('SELECT * FROM print_jobs ORDER BY created_at DESC LIMIT 50').all();
  }
  res.json(jobs);
});
...
app.patch('/api/print-jobs/:id', (req, res) => {
  ...
  db.prepare("UPDATE print_jobs SET status = ?, error = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
    .run(status || job.status, error || null, req.params.id);
